...
namespace InetTools.Controls
{
	...
	public partial class ControlMercuryClient : NotificationControl
	{
		...
		private void OnStart(object sender, EventArgs e)
		{
			...
			// Set the status.
			this.status.Send(CrawlerStatus.StatusType.Busy, "Uploading the traceroute to the Mercury server...", Properties.Resources.Busy_16);
			// Show a message.
			this.ShowMessage(
			...
			catch (Exception exception)
			{
				...
				this.status.Send(CrawlerStatus.StatusType.Normal, "Uploading the traceroute to the Mercury server failed. {0}".FormatWith(exception.Message), Properties.Resources.Error_16);
				// Show a message.
				this.ShowMessage(
		...
		private void OnCallback(AsyncWebResult result)
		{
			...
			catch (WebException exception)
			{
				...
				{
					// Update the status label.
					this.status.Send(CrawlerStatus.StatusType.Normal, "Uploading the traceroute to the Mercury server was canceled.", Properties.Resources.Canceled_16);
					// Show a message.
					this.ShowMessage(
