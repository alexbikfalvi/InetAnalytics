...
namespace InetCommon.Database
{
	...
	public sealed class DbConfigSql : IDisposable, IEnumerable<DbServerSql>
	{
		...
		private IOrderedEnumerable<DbServerSql> orderedServers = null;
		private DbServerSql primary = null;
		...
		public DbConfigSql(IDbConfig config, RegistryKey rootKey, string path)
		{
			...
			foreach (string id in this.key.GetSubKeyNames())
			{
				...
				try
				{
					...
					DbServerSql.DbType type = (DbServerSql.DbType)RegistryExtensions.GetInteger(serverKey.Name, "Type", 0);
		...
		public DbServerSql this[Guid id] { get { return this.servers[id]; } }
		/// <summary>
		...
		/// </summary>
		public DbServerSql Primary { get { return this.primary; } }
		/// <summary>
		...
		public void Reload()
		{
			foreach (DbServerSql server in this.servers.Values)
			{
		...
		{
			return object.ReferenceEquals(this.primary, server);
		}
		...
		public void SetPrimary(DbServerSql server)
		{
			// Save the old primary server.
			DbServerSql oldPrimary = this.primary;
		...
		/// </summary>
		/// <returns>The enumerator.</returns>
		public IEnumerator<DbServerSql> GetEnumerator()
		{
			// Order the server list.
			this.orderedServers = Enumerable.OrderBy<DbServerSql, DateTime>(this.servers.Values, pair => pair.DateCreated);
			...
			)
		{
			...
			if (this.servers.ContainsKey(id)) throw new DbException("Cannot add a new database server. The server ID \'{0}\' already exists.".FormatWith(id));
		...
		public void Remove(DbServerSql server)
		{
			Guid id = server.Id;
			// Check that the server ID exists.
			if (!this.servers.ContainsKey(id)) throw new DbException("Cannot remove the database server. The server with ID \'{0}\' does not exist.".FormatWith(id));
			// If there are more than one server, and this is the primary server, the user must select a different primary server first.
			if ((this.servers.Count > 1) && (this.primary == server)) throw new DbException("Cannot remove the database server. First, select a different primary server.");
		...
		public IAsyncResult RemoveAsync(DbServerSql server, DbServerCallback callback, object userState = null)
		{
			Guid id = server.Id;
			// Check that the server ID exists.
			if (!this.servers.ContainsKey(id)) throw new DbException("Cannot remove the database server. The server with ID \'{0}\' does not exist.".FormatWith(id));
		...
		private void Add(DbServerSql server)
		{
			lock (this.sync)
			{
