...
namespace YtAnalytics.Controls.PlanetLab
{
	...
	{
		private PlRequest request = new PlRequest(PlRequest.RequestMethod.GetNodeTags);
		...
		{
			this.OnTagSet(obj as PlNodeTag);
		}
		...
		protected override void OnUpdate(int id)
		{
			...
			try
			{
				...
				this.BeginRequest(this.request, CrawlerStatic.PlanetLabUserName, CrawlerStatic.PlanetLabPassword, PlNodeTag.GetFilter(PlNodeTag.Fields.NodeTagId, id));
			}
			catch
			{
		...
		protected override void OnCompleteRequest(XmlRpcResponse response)
		{
			...
			if ((null == response.Fault) && (null != response.Value))
			{
				...
				PlList<PlNodeTag> tags = PlList<PlNodeTag>.Create(response.Value as XmlRpcArray);
		...
		protected virtual void OnTagSet(PlNodeTag tag)
		{
