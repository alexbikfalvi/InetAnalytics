...
{
	/// <summary>
	/// A class representing a gd:rating atom.
	/// </summary>
	[Serializable]
	public sealed class AtomGdRating : Atom
	{
		...
		private AtomGdRating(XElement element)
			: base(xmlPrefix, xmlName, element)
		{
			// Set the attributes.
			this.Min = element.Attribute("min").Value.ToInt();
			this.Max = element.Attribute("max").Value.ToInt();
			this.NumRaters = element.Attribute("numRaters").Value.ToInt();
			this.Average = element.Attribute("average").Value.ToDouble();
		}
		...
		public static AtomGdRating Parse(XElement element, bool mandatory)
		{
			...
			return new AtomGdRating(element);
		}
		...
		/// <param name="mandatory">Specified whether this element is mandatory.</param>
		/// <returns>The atom instance.</returns>
		public static AtomGdRating ParseChild(XElement element, bool mandatory)
		{
			...
			try
			{
				...
				return AtomGdRating.Parse(element.Element(AtomGdRating.xmlPrefix, AtomGdRating.xmlName), mandatory);
			}
		...
		public int NumRaters { get; private set; }
