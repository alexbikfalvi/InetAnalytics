...
namespace InetApi.YouTube.Api.V2
{
	...
	/// </summary>
	public class YouTubeRequestVideo : YouTubeRequest
	{
		/// <summary>
		/// Conversion class for an asynchronous operation returning a video.
		/// </summary>
		public class VideoRequestFunction : IAsyncFunction<Video>
		{
			public Video GetResult(string data)
			{
				// Parse the string data.
				return new Video(AtomEntryVideo.Parse(data));
			}
		}

		private VideoRequestFunction funcConvert = new VideoRequestFunction();

		/// <summary>
		/// A request to the YouTube API for a video.
		/// </summary>
		/// <param name="settings">The request settings.</param>
		public YouTubeRequestVideo(YouTubeSettings settings)
			: base(settings)
		{
		}
		...
		public new Video End(IAsyncResult result, out object state)
		{
