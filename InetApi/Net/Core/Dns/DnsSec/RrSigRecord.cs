...
namespace InetApi.Net.Core.Dns
{
	...
	public class RrSigRecord : DnsRecordBase
	{
		...
		{
			get { return 20 + this.SignersName.Length + this.Signature.Length; }
		}
		...
		{
			DnsMessageBase.EncodeUShort(messageData, ref currentPosition, (ushort)this.TypeCovered);
			messageData[currentPosition++] = (byte)this.Algorithm;
			messageData[currentPosition++] = this.Labels;
			DnsMessageBase.EncodeInt(messageData, ref currentPosition, this.OriginalTimeToLive);
			RrSigRecord.EncodeDateTime(messageData, ref currentPosition, this.SignatureExpiration);
			RrSigRecord.EncodeDateTime(messageData, ref currentPosition, this.SignatureInception);
			DnsMessageBase.EncodeUShort(messageData, ref currentPosition, this.KeyTag);
			DnsMessageBase.EncodeDomainName(messageData, offset, ref currentPosition, this.SignersName, false, null);
			DnsMessageBase.EncodeByteArray(messageData, ref currentPosition, this.Signature);
		}
